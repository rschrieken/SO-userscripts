...
export class UserListController extends Stacks.StacksController {
  ...
  updateUsers(): void {
    ...
    const hidrationRows = new Map(
      this.userRowTargets.reduce(
        (entries, row) =>
          row.dataset.hidrated === 'true'
            ? entries
            : [...entries, [parseInt(row.dataset.uid || '0'), row]],
        [] as [number, HTMLDivElement][]
      )
    )
    if (hidrationRows.size === 0) return
    window.requestAnimationFrame(async () => {
      for await (const user of fetchUsers([...hidrationRows.keys()], true)) {
        const userRow = hidrationRows.get(user.user_id)
        if (!userRow) continue
        const firstChild =
